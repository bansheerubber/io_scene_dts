...
def export_material(mat, shape):
    ...
    if mat.torque_props.use_shadeless:
        flags |= Material.SelfIlluminating
    if mat.torque_props.use_transparency:
...
def export_empty_node(lookup, shape, select_object, ob, parent=-1):
    ...
    node.armature = None
    node.bl_ob = ob
    node.animation_data = ob.animation_data
    node.matrix = ob.matrix_local
    node.matrix_world = ob.matrix_world
    node.rotation_mode = ob.rotation_mode
...
def export_bones(lookup, shape, armature, bones, parent=-1):
    for bone in bones:
        ...
        node.bl_ob = bone
        node.animation_data = bone.animation_data
        node.matrix = mat
        node.rotation_mode = bone.rotation_mode
    ...
    for bobj in scene.objects:
        ...
        if bobj.users_collection:
            ...
            if "collection" in bobj.users_collection[0].name.lower():
                ...
                lod_name = bobj.users_collection[0].name
                print(lod_name)
    ...
    for object, lods in scene_objects.values():
        ...
        for i in range(object.numMeshes):
            ...
            if lod_name in lods:
                ...
                # bpy.data.meshes.remove(mesh) # RIP!    gyt: is this needed anymore?
